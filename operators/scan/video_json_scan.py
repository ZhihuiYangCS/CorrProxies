...
from paths import VIDEO_UCF101_TRAIN_DATA_PATH
from records.record import Record


class VideoJsonScan(JsonScan):
    """
    An interface to scan all video json data
    """
        ...
        if not self._file_reader.closed:
            ...
            while line.strip():
                if line.startswith("{"):
                    ...
                    # record['frames'] = list(json_data['frames'])
                    self.output_num += 1
                    break
                else:
                    line = self._file_reader.readline()
    ...
    def get_new_object(self, previous_operators=None, operator_name: str = ""):
        ...
        """
        return VideoJsonScan(read_path=self._read_path, file_mode=self.file_mode)
