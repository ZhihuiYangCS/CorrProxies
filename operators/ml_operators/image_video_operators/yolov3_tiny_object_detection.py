...
rootpath.append()
from paths import YOLO_DIR, UCF101_FRAMES_PATH
import sys
...
from paths import VIDEO_UCF101_TRAIN_DATA_PATH, YOLO_TINY_WEIGHTS_PATH
from records.record import Record
from operators.operator_base.operator_parallel import OperatorParallel


class Yolov3TinyObjectDetection(OperatorParallel):
    """
    https://github.com/ultralytics/yolov3
    https://pjreddie.com/darknet/yolo/
    This class implments the yolov3 object detection operator for videos
    """

    def __init__(self):
        super(Yolov3TinyObjectDetection, self).__init__()

    def get_operator_cost(self):
        """return operator time cost per record, in ms/record
            :raises ValueError
        """
        if Yolov3TinyObjectDetection.operator_cost is None:
            if self.process_count == 0:
                print("Getting operator cost: process_count = " + str(
                    self.process_count) + "\tYou may forget add processing statistic informations ...")
                Yolov3TinyObjectDetection.operator_cost = None
            else:
                Yolov3TinyObjectDetection.operator_cost = self.process_time / self.process_count

    def load_model(self) -> Any:
        time1 = time.time()
        model = load_model(weights_path=YOLO_TINY_WEIGHTS_PATH)
    ...
    def get_new_object(self, previous_operators=None, operator_name: str = ""):
        ...
        """
        return Yolov3TinyObjectDetection()
...
def operator_name_test():
    operator = Yolov3TinyObjectDetection()
    operator_model = operator.load_model()
    print(operator)
...
def operator_cost_test():
    ...
    operator_new = Yolov3TinyObjectDetection()
