...
from utility.constant import TARGET_ACCURACY_DEFAULT, SCHEME_DEFAULT, OPT_THREAD_NUM_DEFAULT, EXE_THREAD_NUM_DEFAULT, \
    ...
    COCO_TYPE, COCO_EXE_BATCH_SIZE, UCF101_EXE_BATCH_SIZE, TWITTER_EXE_BATCH_SIZE, UCF101_TYPE
from utility.utility import batch_records_delete_ml_operator_results


class WorkflowExp2(WorkflowBase):
    """
    ...
    def set_operators_cost(self):
        ...
        if self.workflow_type == UCF101_TYPE and self.opt_thread_num == 0:
            batch_output = self.preprocessor.process_batch(batch=batch_records,
                                                           model=self.models[self.preprocessor.operator_name])
        else:
        ...
        for ml_filter in self.get_ml_filters():
            if self.workflow_type == UCF101_TYPE and self.opt_thread_num == 0:
                batch_output = ml_filter.process_batch(batch=batch_records, model=self.models[
                    ml_filter.ml_predicator.ml_operator.operator_name])
            elif self.workflow_type == TWITTER_TYPE:
    ...
    def optimization(self):
        ...
        scan_opt_output_num_list.append(workflow_reorder.workflow[0].output_num)
    ...
    def execution(self, schemes_list: List[int], pp_filters_list: List[List[PPFilter]],
        ...
        original_count = [0] * (len(ml_filters_list[0]) + 1)
        ...
        while batch_records:
            ...
            if self.workflow_type == UCF101_TYPE and self.exe_thread_num == 0:
                batch_output = self.preprocessor.process_batch(batch=batch_records,
                                                               model=self.models[self.preprocessor.operator_name])
            else:
                batch_output = self.preprocessor.multiple_processes_batch(batch=batch_records,
                                                                          num_process=self.exe_thread_num)
            batch_records = batch_output.returned_batch
            block_records = batch_records.copy()
            original_count[0] += len(batch_records)
            for index, ml_filter in enumerate(ml_filters_list[0]):
                if self.workflow_type == UCF101_TYPE and self.exe_thread_num == 0:
                    ml_batch_output = ml_filter.process_batch(batch=block_records, model=self.models[
                        ml_filter.ml_predicator.ml_operator.operator_name])
                elif self.workflow_type == TWITTER_TYPE:
                ...
                original_count[index + 1] += len(block_records)
        ...
        for i in range(1, len(schemes_list)):
            ...
            """
            scheme_count = [0] * (len(pp_filters_list[i]) + len(ml_filters_list[i]) + 1)
            scheme_count[0] += len(all_input_records_copy)
            for index, pp_filter in enumerate(pp_filters_list[i]):
                scheme_cost += len(all_input_records_copy) * pp_filter.accuracy_info_dict.infer_cost
                if self.workflow_type == UCF101_TYPE and self.exe_thread_num == 0:
                    pp_batch_output = pp_filter.process_batch(batch=all_input_records_copy,
                                                              model=self.models[self.preprocessor.operator_name])
                else:
                ...
                scheme_count[index + 1] += len(all_input_records_copy)

            for index, ml_filter in enumerate(ml_filters_list[i]):
                if self.workflow_type == UCF101_TYPE and self.exe_thread_num == 0:
                    ml_batch_output = ml_filter.process_batch(batch=all_input_records_copy, model=self.models[
                        ml_filter.ml_predicator.ml_operator.operator_name])
                elif self.workflow_type == TWITTER_TYPE:
                ...
                scheme_count[index + 1 + len(pp_filters_list[i])] += len(all_input_records_copy)
            ...
            print("scheme count " + str(schemes_list[i]) + " = " + str(scheme_count))

        print("original scheme count = " + str(original_count))
